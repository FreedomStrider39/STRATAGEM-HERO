        ...
        <AnimatePresence mode="wait">
          {gameState === "idle" && (
            <motion.div 
              ...
              onClick={startGame}
            >
              <h1 className="text-5xl md:text-8xl font-black tracking-tighter text-white mb-4 italic drop-shadow-[0_0_30px_rgba(255,255,255,0.3)]">
                STRATAGEM HERO
              </h1>
              <div className="h-1 w-48 md:w-64 bg-yellow-400 mb-8 shadow-[0_0_15px_rgba(250,204,21,0.8)]" />
              <motion.p 
          ...
          {gameState === "playing" && (
            <motion.div 
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="w-full h-full flex flex-col md:grid md:grid-cols-[200px_1fr_200px] items-center gap-2 md:gap-4 z-10 py-4 md:py-12"
            >
              <div className="hidden md:flex flex-col items-center justify-center border-r-2 border-white/10 h-full">
                <span className="text-[#4ade80] text-xl font-bold tracking-[0.2em] mb-2">ROUND</span>
                <span className="text-yellow-400 text-7xl font-black leading-none text-glow-yellow">
                  {level}
                </span>
              </div>
              ...
              <div className="flex flex-col items-center justify-between h-full w-full max-w-[700px] mx-auto relative">
                  ...
                  <AnimatePresence>
                    {isDisrupted && (
                      <motion.div 
                        ...
                        exit={{ opacity: 0 }}
                        className="flex items-center justify-center gap-2 bg-purple-600/20 border border-purple-500/50 py-1 px-4 backdrop-blur-sm"
                      >
                        <AlertTriangle className="w-4 h-4 text-purple-400 animate-pulse" />
                        <span className="text-purple-400 text-[10px] md:text-xs font-bold tracking-[0.1em] animate-pulse text-center">
                    ...
                    {showDisruptorDestroyed && (
                      <motion.div 
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0 }}
                        className="flex items-center justify-center gap-2 bg-orange-600/20 border border-orange-500/50 py-1 px-4 backdrop-blur-sm"
                      >
                        <CheckCircle2 className="w-4 h-4 text-orange-400" />
                        <span className="text-white text-[10px] md:text-xs font-bold tracking-[0.1em] text-center">
                          DISRUPTOR DESTROYED
                        </span>
                      </motion.div>
                    )}
                ...
                <div className="w-full px-4 mt-4">
                  <div className="relative h-4 md:h-8 bg-black/60 border-2 border-white/20 overflow-hidden">
                    <motion.div 
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <span className="text-[8px] md:text-[10px] font-bold tracking-[0.5em] text-white/40">REMAINING TIME</span>
                    </div>
                  </div>
                </div>
              ...
              <div className="hidden md:flex flex-col items-center justify-center border-l-2 border-white/10 h-full">
                ...
                <span className="text-[#4ade80] text-xl font-bold tracking-[0.2em] mt-2">SCORE</span>
              </div>
            </motion.div>
          )}
          ...
          {gameState === "break" && (
            <motion.div 
              ...
              exit={{ opacity: 0 }}
              className="flex flex-col items-center text-center z-30 bg-black/60 p-8 md:p-12 border-y-4 border-yellow-400 w-full"
            >
              <h2 className="text-4xl md:text-7xl font-black text-yellow-400 mb-4 italic tracking-tighter text-glow-yellow">
                ROUND COMPLETE
              </h2>
              <div className="h-1 w-32 md:w-48 bg-white/20 mb-6" />
              <p className="text-xl md:text-3xl font-bold text-white mb-2 tracking-widest">PREPARING NEXT WAVE</p>
              <div className="text-3xl md:text-5xl font-black text-yellow-400 animate-pulse">
          ...
          {gameState === "gameover" && (
            <motion.div 
              ...
              onClick={startGame}
            >
              <h2 className="text-4xl md:text-7xl font-black text-red-500 mb-6 md:mb-8 italic tracking-tighter drop-shadow-[0_0_20px_rgba(239,68,68,0.5)]">
                MISSION FAILED
              </h2>

              <div className="grid grid-cols-2 gap-x-6 md:gap-x-24 gap-y-2 md:gap-y-4 w-full max-w-3xl bg-black/40 p-6 md:p-8 border-2 border-white/10">
                <div className="text-left">
                  <p className="text-[#4ade80] text-sm md:text-xl font-bold tracking-widest">ROUND BONUS</p>
                </div>
                <div className="text-right">
                  <p className="text-yellow-400 text-lg md:text-3xl font-black">{stats.roundBonus}</p>
                </div>
                ...
                <div className="text-right">
                  <p className="text-yellow-400 text-lg md:text-3xl font-black">{stats.timeBonus}</p>
                </div>
                ...
                <div className="text-right">
                  <p className="text-yellow-400 text-lg md:text-3xl font-black">{stats.perfectBonus}</p>
                </div>

                <div className="col-span-2 h-[1px] md:h-[2px] bg-white/20 my-1 md:my-2" />

                <div className="text-left">
                  <p className="text-[#4ade80] text-lg md:text-3xl font-black tracking-[0.1em] md:tracking-[0.2em]">TOTAL SCORE</p>
                </div>
                <div className="text-right">
                  <p className="text-yellow-400 text-2xl md:text-6xl font-black text-glow-yellow">{stats.totalScore}</p>
                </div>

                <div className="col-span-2 flex justify-center mt-2 pt-2 border-t border-white/10">
                  <p className="text-white/40 text-[10px] md:text-sm tracking-[0.2em] md:tracking-[0.3em]">
