  ...
  return (
    <div className="h-dynamic-screen bg-[#0a0c0c] text-white font-sans selection:bg-yellow-400 selection:text-black flex items-center justify-center p-0 overflow-hidden">
      <div className="w-full h-full max-w-full bg-[#121616] relative flex flex-col items-center justify-center px-1 md:px-12 crt-screen border-x-[2px] md:border-x-[8px] border-[#1a1f1f]">
        ...
        <AnimatePresence mode="wait">
          ...
          {gameState === "playing" && (
            <motion.div 
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="w-full h-full flex flex-col items-center justify-between z-10 py-1 md:py-12"
            >
              <div className="w-full max-w-[1100px] mx-auto flex flex-col h-full">
                {/* Status Alerts */}
                <div className="h-6 md:h-12 flex flex-col gap-1 mb-1">
                  <AnimatePresence>
                    {isDisrupted && (
                      <motion.div 
                        ...
                        exit={{ opacity: 0 }}
                        className="flex items-center justify-center gap-2 bg-purple-600/30 border border-purple-500/50 py-0.5 md:py-2 px-4 backdrop-blur-md"
                      >
                        <AlertTriangle className="w-3 h-3 md:w-6 md:h-6 text-purple-400 animate-pulse" />
                        <span className="text-purple-400 text-[7px] md:text-lg font-bold tracking-[0.1em] animate-pulse text-center">
                    ...
                    {showDisruptorDestroyed && (
                      <motion.div 
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0 }}
                        className="flex items-center justify-center gap-2 bg-orange-600/30 border border-orange-500/50 py-0.5 md:py-2 px-4 backdrop-blur-md"
                      >
                        <CheckCircle2 className="w-3 h-3 md:w-6 md:h-6 text-orange-400" />
                        <span className="text-white text-[7px] md:text-lg font-bold tracking-[0.1em] text-center">
                          DISRUPTOR DESTROYED
                        </span>
                      </motion.div>
                    )}
                  ...
                  <AnimatePresence>
                    {combo > 1 && (
                      <motion.div
                        ...
                        className="absolute top-1/2 right-1 md:right-12 flex flex-col items-center z-20"
                      >
                        <div className="flex items-center gap-1 text-yellow-400">
                          <Zap className="w-3 h-3 md:w-10 md:h-10 fill-yellow-400" />
                          <span className="text-lg md:text-6xl font-black italic text-glow-yellow">x{combo}</span>
                        </div>
                        <span className="text-[6px] md:text-sm font-bold tracking-[0.1em] text-white/60">COMBO</span>
                      </motion.div>
                    )}
                  </AnimatePresence>
                </div>
                ...
                {/* Controls & Timer */}
                <div className="w-full flex flex-col items-center gap-1 md:gap-8">
          ...
          {gameState === "break" && (
            <motion.div 
              ...
              exit={{ opacity: 0 }}
              className="flex flex-col items-center text-center z-30 bg-black/60 p-4 md:p-20 border-y-[2px] md:border-y-[8px] border-yellow-400 w-full"
            >
              <h2 className="text-2xl md:text-7xl font-black text-yellow-400 mb-1 md:mb-4 italic tracking-tighter text-glow-yellow leading-none">
                ROUND COMPLETE
              </h2>
              <div className="h-0.5 w-16 md:h-1.5 md:w-[20rem] bg-white/20 mb-4 md:mb-8" />
              <p className="text-[10px] md:text-4xl font-bold text-white mb-2 md:mb-6 tracking-widest">PREPARING NEXT WAVE</p>
              <div className="text-2xl md:text-6xl font-black text-yellow-400 animate-pulse">
          ...
          {gameState === "gameover" && (
            ...
            >
              ...
              <div className="flex items-center gap-2 md:gap-4 mb-2 md:mb-6 bg-yellow-400/10 border border-yellow-400/30 px-3 md:px-6 py-1 md:py-3">
                <Trophy className="text-yellow-400 w-4 h-4 md:w-8 md:h-8" />
                <div className="flex flex-col">
                  <span className="text-[6px] md:text-xs text-white/60 font-bold tracking-widest">CURRENT RANK</span>
                  <span className="text-xs md:text-2xl font-black text-yellow-400 italic">{getRank(level)}</span>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-x-2 md:gap-x-32 gap-y-1 md:gap-y-8 w-full max-w-[1100px] bg-black/40 p-3 md:p-12 border md:border-[4px] border-white/10">
                <div className="text-left">
                  <p className="text-[#4ade80] text-[8px] md:text-3xl font-bold tracking-widest">ROUND BONUS</p>
                </div>
                <div className="text-right">
                  <p className="text-yellow-400 text-sm md:text-5xl font-black">{stats.roundBonus}</p>
                </div>

                <div className="text-left">
                  <p className="text-[#4ade80] text-[8px] md:text-3xl font-bold tracking-widest">TIME BONUS</p>
                </div>
                <div className="text-right">
                  <p className="text-yellow-400 text-sm md:text-5xl font-black">{stats.timeBonus}</p>
                </div>

                <div className="text-left">
                  <p className="text-[#4ade80] text-[8px] md:text-3xl font-bold tracking-widest">MAX COMBO</p>
                </div>
                <div className="text-right">
                  <p className="text-yellow-400 text-sm md:text-5xl font-black">{stats.maxCombo}</p>
                </div>

                <div className="col-span-2 h-[1px] md:h-[2px] bg-white/20 my-1 md:my-6" />

                <div className="text-left">
                  <p className="text-[#4ade80] text-[10px] md:text-4xl font-black tracking-[0.1em]">TOTAL SCORE</p>
                </div>
                <div className="text-right">
                  <p className="text-yellow-400 text-lg md:text-7xl font-black text-glow-yellow leading-none">{stats.totalScore}</p>
                </div>

                <div className="col-span-2 flex justify-center mt-1 pt-1 border-t border-white/10">
                  <p className="text-white/40 text-[8px] md:text-2xl tracking-[0.1em]">
                    HIGH SCORE: <span className="text-yellow-400/60">{highScore}</span>
                  </p>
                </div>
              </div>

              <p className="mt-4 md:mt-16 text-white/40 text-[10px] md:text-2xl font-bold animate-pulse tracking-[0.1em] text-center">
        ...
        <div className="absolute bottom-0.5 md:bottom-4 left-0 right-0 flex justify-center z-30 opacity-30 scale-[0.4] md:scale-100">
          <MadeWithDyad />
        </div>
      </div>
    </div>
