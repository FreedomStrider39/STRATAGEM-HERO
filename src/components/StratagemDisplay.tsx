...
const CustomArrow = ({ direction, completed, isDisrupted }: { direction: Direction, completed: boolean, isDisrupted?: boolean }) => {
  ...
  return (
    <motion.div 
      ...
      className={cn(
        "transition-all duration-75 w-12 h-12 md:w-14 md:h-14 flex items-center justify-center flex-shrink-0",
...
const StratagemDisplay: React.FC<StratagemDisplayProps> = ({ 
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-5xl mx-auto px-2 h-full justify-center py-2 md:py-0 gap-4 md:gap-6">
      ...
      <div className="flex md:hidden items-center justify-between w-full px-6">
        <div className="flex flex-col items-start">
          ...
          <span className="text-yellow-400 text-4xl font-black leading-none text-glow-yellow">{round}</span>
        </div>
        <div className="flex flex-col items-end">
          <span className="text-white/60 text-[10px] font-bold tracking-[0.2em]">SCORE</span>
          <span className="text-yellow-400 text-4xl font-black leading-none text-glow-yellow">{score}</span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center w-full gap-2 md:gap-12">
        ...
        <div className="hidden md:flex flex-col items-center w-40 flex-shrink-0">
          ...
          <span className="text-yellow-400 text-7xl font-black leading-none text-glow-yellow">{round}</span>
        </div>

        {/* Center: Main Icon and Upcoming Queue */}
        <div className="flex items-end gap-2 md:gap-6 overflow-hidden">
          <div className={cn(
            "w-32 h-32 md:w-56 md:h-56 border-2 md:border-[6px] p-1 md:p-3 bg-black/40 relative overflow-hidden transition-colors duration-500 flex-shrink-0",
          ...
          <div className="flex gap-1.5 md:gap-4 pb-1 md:pb-3">
            {queue.slice(1, 4).map((nextStrat, idx) => (
              <div key={idx} className="w-10 h-10 md:w-24 md:h-24 opacity-30 grayscale brightness-75 relative overflow-hidden border border-white/5 flex-shrink-0">
                <StratagemIcon 
        ...
        <div className="hidden md:flex flex-col items-center w-40 flex-shrink-0">
          ...
          <span className="text-yellow-400 text-7xl font-black leading-none text-glow-yellow">{score}</span>
        </div>
      </div>

      {/* Name Bar */}
      <div className={cn(
        "w-full py-3 md:py-5 px-4 md:px-12 transition-all duration-500",
        isDisrupted ? "bg-purple-900/90 border-y-2 border-purple-500/50" : "bg-yellow-400"
      )}>
        <h2 className={cn(
          "text-lg md:text-3xl font-black text-center tracking-[0.1em] md:tracking-[0.2em] min-h-[1.5rem] md:min-h-[2.5rem] flex items-center justify-center",
      ...
      {/* Arrows */}
      <div className={cn(
        "flex flex-wrap justify-center gap-2 md:gap-3 transition-transform duration-75",
